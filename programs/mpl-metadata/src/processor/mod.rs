...
use crate::{
    ...
    state::{Key, Metadata, TokenStandard, TokenState, DISCRIMINATOR_INDEX, TOKEN_STATE_INDEX},
    utils::SPL_TOKEN_2022_ID,
};
...
pub fn process_instruction<'a>(
    ...
    //     msg!("IX: Collect");
    //     // fee::process_collect_fees(program_id, accounts)
    //     Ok(())
    // }
    ...
    //     msg!("IX: Close Accounts");
    //     // close::process_close_accounts(program_id, accounts)
    //     Ok(())
    // }
    // _ => {
...
fn incompatible_accounts(
    ...
    for account_info in accounts {
        ...
        if account_info.owner() == program_id && !account_info.data_is_empty() {
            let discriminator = account_info.try_borrow_data()?[DISCRIMINATOR_INDEX];
        ...
        } else if account_info.key() == &SPL_TOKEN_2022_ID
            || account_info.owner() == &SPL_TOKEN_2022_ID
        {
            return Ok(true);
        }
    }
...
fn is_locked(program_id: &Pubkey, accounts: &[AccountInfo]) -> bool {
    for account_info in accounts {
        // checks the account is owned by Token Metadata and it has data
        if account_info.owner() == program_id && !account_info.data_is_empty() {
            let data = account_info.try_borrow_data().map_err(|_| false).ok();
            // checks if the account is a Metadata account
